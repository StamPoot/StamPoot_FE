...
fun GalleryReadScreen(galleryViewModel: GalleryViewModel = hiltViewModel()) {
    ...
    BaseScreen { paddingValue ->
        ...
        Column(
            ...
            TitleLargeText(text = Formatter.dateToString(readingDiary.date))
            TitleLargeText(text = readingDiary.location.location)
            TitleLargeText(text = readingDiary.title)
            ImagesLayout(selectedImages = readingDiary.photoURLs.map { Uri.parse(it) })
            TitleLargeText(text = readingDiary.message)
        }
    }
}
